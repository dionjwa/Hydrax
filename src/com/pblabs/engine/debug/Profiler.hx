 ...
 */
	...
	class Profiler
	{
		...
		{
			var p :V8Profiler = Node.require('v8-profiler');
			p.startProfiling(blockName);
		}
		...
		{
			var p :V8Profiler = Node.require('v8-profiler');
			p.stopProfiling(blockName);
		}
		
		public static function takeSnapshot (key :String) :Void
		{
			trace("takeSnapshot not yet implemented");
			// var p :V8Profiler = Node.require('v8-profiler');
			// p.takeSnapshot(key);
		}
